...
var rule = {
    ...
    搜索: `js:
        var DEBUG = true; // set to false once search works
        var dbg = [];
        ...
        function plain(h) {
            return String(h).replace(/<[^>]+>/g, ' ').replace(/\\s+/g, ' ').slice(0, 60);
        }
        var html = request(input);
        var d = parseList(html);
        dbg.push('1.first response: len=' + String(html).length + ' text="' + plain(html) + '"');
        if (d.length === 0 && /verify/.test(html)) {
            for (var i = 0; i < 3 && d.length === 0; i++) {
                try {
                    ...
                    var ck = sc.split(';')[0];
                    dbg.push('2.try' + i + ' cookie=' + (ck || 'NONE') + ' imgLen=' + String(r.body || '').length + ' keys=' + Object.keys(r).join(','));
                    var code = String(post('https://api.nn.ci/ocr/b64/text', {body: r.body})).replace(/\\s/g, '');
                    dbg.push('3.ocr="' + code.slice(0, 12) + '"');
                    var vr = request(HOST + '/index.php/ajax/verify_check?type=search&verify=' + code, {headers: {Cookie: ck}});
                    dbg.push('4.check=' + String(vr).slice(0, 50));
                    html = request(input, {headers: {Cookie: ck}});
                    d = parseList(html);
                    dbg.push('5.retry: len=' + String(html).length + ' results=' + d.length + ' text="' + plain(html) + '"');
                } catch (e) {
        ...
        if (d.length === 0 && DEBUG) {
            d.push({title: '[debug] ' + dbg.join(' | '), img: '', desc: dbg.join(' | '), url: ''});
        }
        setResult(d);
    `,
}